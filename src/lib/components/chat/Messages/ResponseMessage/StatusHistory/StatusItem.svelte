		...
		{#if status?.action === 'web_search' && (status?.urls || status?.items)}
			<WebSearchResults status={status as any}>
				<div class="flex flex-col justify-center -space-y-0.5">
					<div
						class="{(done || status?.done) === false ? 'shimmer' : ''} {compact
		...
		{:else if status?.action === 'knowledge_search'}
			<div class="flex flex-col justify-center -space-y-0.5">
				<div
					class="{(done || status?.done) === false ? 'shimmer' : ''} {compact
						? 'text-sm text-gray-600 dark:text-gray-300'
						: 'text-base text-gray-500 dark:text-gray-500'} line-clamp-1 text-wrap"
				>
			...
			<div class="flex flex-col justify-center -space-y-0.5">
				...
				<div class=" flex gap-1 flex-wrap mt-2">
					{#each status.queries as query, idx (query)}
						<div
							class="bg-gray-50 dark:bg-gray-850 flex rounded-lg py-1 px-2 items-center gap-1 {compact
								? 'text-[11px]'
								: 'text-xs'}"
						>
							<div>
