<script lang="ts">
	...
	import { importHermesSession, type HermesRuntime } from '$lib/apis/hermes';
	...
	$: currentHermesModelSourceDetail =
		currentHermesModelSourceKind === 'session'
			? runtimeDiffersFromSession
				? $i18n.t('Linked session differs from the active Hermes runtime.')
				: $i18n.t('Pinned in linked session')
			: currentHermesModelSourceKind === 'profile'
...
<div class="flex h-full min-h-0 flex-col px-2 py-2">
	{#if mode === 'context'}
		...
		>
			<div class="flex items-start justify-between gap-3">
				<div class="min-w-0">
					...
					<div class="mt-1 text-xs leading-5 text-gray-500 dark:text-gray-400">
						{$i18n.t(
							'Current Hermes context for this chat, including model source, linked session, and attached files.'
						)}
					</div>
				</div>
				<div
				...
				>
					{$i18n.t(
						currentHermesSessionItem?.imported_chat_id ? 'Imported session' : 'Live runtime'
					)}
				</div>
			</div>

			<div class="mt-3 rounded-2xl bg-gray-50/80 px-4 py-3 dark:bg-gray-950/60">
				<div class="text-[11px] uppercase tracking-[0.08em] text-gray-400 dark:text-gray-500">
					{$i18n.t('Context summary')}
				</div>
				<div class="mt-1 text-sm font-medium leading-6 text-gray-800 dark:text-gray-100">
					{currentHermesSummaryLine}
				</div>
				{#if currentHermesSourceLine && currentHermesSourceLine !== currentHermesSummaryLine}
					<div class="mt-1 text-xs leading-5 text-gray-500 dark:text-gray-400">
						{currentHermesSourceLine}
					</div>
				{/if}
			...
			<div
				class="mt-3 rounded-xl border border-gray-100/80 bg-white/70 px-3 py-2.5 dark:border-gray-800/80 dark:bg-gray-900/40"
			>
				<div class="flex items-center justify-between gap-3">
					<div class="min-w-0">
						<div class="text-[11px] uppercase tracking-[0.08em] text-gray-400 dark:text-gray-500">
							{$i18n.t('Context details')}
						</div>
						<div class="mt-1 text-xs leading-5 text-gray-500 dark:text-gray-400">
							{currentHermesModelSourceDetail}
						</div>
					...
					<div
						class="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-[11px] text-gray-500 dark:bg-gray-800 dark:text-gray-300"
					>
						{$i18n.t('Messages')}: {messages.length}
					</div>
				</div>

				<div class="mt-2 flex flex-wrap gap-1.5 text-[11px] text-gray-500 dark:text-gray-400">
					<div class="rounded-full bg-gray-50 px-2 py-0.5 dark:bg-gray-950/70">
						{$i18n.t('Assistant replies')}: {assistantCount}
					</div>
					<div class="rounded-full bg-gray-50 px-2 py-0.5 dark:bg-gray-950/70">
						{$i18n.t('User turns')}: {userCount}
					</div>
					<div class="rounded-full bg-gray-50 px-2 py-0.5 dark:bg-gray-950/70">
						{$i18n.t('Operational turns')}: {operationalTurns}
					</div>
					<div class="rounded-full bg-gray-50 px-2 py-0.5 dark:bg-gray-950/70">
						{$i18n.t('Attached files')}: {chatFiles.length}
					</div>
					<div class="rounded-full bg-gray-50 px-2 py-0.5 dark:bg-gray-950/70">
		...
		<div class="mt-2 flex flex-col gap-1.5 px-1 pb-2">
			<div
				class="rounded-xl border border-gray-100/70 bg-gray-50/70 px-3 py-2.5 dark:border-gray-800/70 dark:bg-gray-950/40"
			>
				<div class="flex items-center justify-between gap-3">
					<div class="min-w-0">
						<div
			...
			>
				...
				<div class="mt-3 flex items-center gap-2">
					<div class="flex flex-1 items-center rounded-xl bg-gray-50 px-3 py-2 dark:bg-gray-900/70">
						...
						<input
							...
							placeholder={$i18n.t('Search Hermes sessions')}
						/>
					</div>
				</div>
				...
				{#if sessionFilters.length > 1}
					<div class="mt-3 border-t border-gray-100/80 pt-3 dark:border-gray-800/80">
						<div
							class="mb-2 text-[11px] uppercase tracking-[0.08em] text-gray-400 dark:text-gray-500"
						>
							{$i18n.t('Scope')}
						</div>
				...
				{#if auxiliarySessions.length > 0}
					...
					>
						<div class="min-w-0">
							...
							{:else}
								{auxiliarySessions.length}
								{$i18n.t('auxiliary or system sessions are hidden by default.')}
							{/if}
						</div>
