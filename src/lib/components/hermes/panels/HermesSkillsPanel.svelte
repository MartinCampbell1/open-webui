<script lang="ts">
	...
	$: scopeStats = [
		{
			label: $i18n.t('Profile skills'),
			value: profileSkillCount ?? '—'
		},
		{
			label: $i18n.t('Skills in view'),
			value: total
		},
		{
			label: $i18n.t('Enabled on page'),
...
<div class="flex h-full min-h-0 flex-col px-2 py-2">
	...
	<div class="px-2 pb-2">
		<div
			class="rounded-xl border border-gray-100/70 bg-white/70 px-3 py-2.5 dark:border-gray-800/70 dark:bg-gray-900/45"
		>
			<div class="flex items-start justify-between gap-3">
				<div class="min-w-0">
					...
					<div class="mt-1 text-sm font-medium text-gray-700 dark:text-gray-200">
						{$i18n.t('Current profile, filter, and page')}
					</div>
					<div class="mt-1 text-xs leading-5 text-gray-500 dark:text-gray-400">
						{$i18n.t(
							'The active profile sets the baseline. The list below reflects the current filter and page.'
						)}
					</div>
				</div>
			...
			<div
				class="mt-3 grid grid-cols-3 overflow-hidden rounded-lg border border-gray-100/70 bg-gray-50/50 dark:border-gray-800/70 dark:bg-gray-950/35"
			>
				{#each scopeStats as stat, index}
					<div
						class={`px-3 py-2 ${
							index < scopeStats.length - 1
								? 'border-r border-gray-100/70 dark:border-gray-800/70'
								: ''
						}`}
					>
		...
		{:else if items.length === 0}
			<div class="px-3 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
				...
				{:else if query || viewOption}
					{$i18n.t('No skills match this view.')}
				{:else}
					{$i18n.t('No skills are visible in this workspace.')}
				{/if}
			</div>
		{:else}
