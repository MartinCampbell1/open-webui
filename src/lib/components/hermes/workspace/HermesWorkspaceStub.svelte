...
<script lang="ts">
	...
	$: if (active) {
		const visibleItemCount = canBrowseWorkspace
...
<div class="flex h-full min-h-0 flex-col px-2 py-2">
		...
		>
			...
			<div class="mt-2 flex flex-col gap-1.5">
				{#each availableHermesWorkspaces as workspace}
					<button
						type="button"
						class="focus-ring w-full rounded-lg border px-2.5 py-2 text-left transition disabled:cursor-not-allowed disabled:opacity-60 {workspace.is_active
	...
	{:else if selectedItem}
		<div class="flex flex-1 min-h-0 flex-col overflow-hidden px-2">
			<div
				class="mx-2 rounded-xl border border-gray-100/80 bg-white/90 dark:border-gray-800/80 dark:bg-gray-900/50 md:hidden"
			>
				<div class="flex flex-wrap gap-1.5 border-b border-gray-100 px-3 py-2 dark:border-gray-800">
					<button
						class="focus-ring touch-target-compact rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-medium text-gray-600 transition hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
						on:click={closePreview}
					>
						{$i18n.t('Back to list')}
					</button>
					<button
						class="focus-ring touch-target-compact rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-medium text-gray-600 transition hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
			...
			<div
				class="hidden min-h-0 flex-1 gap-2 md:grid md:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]"
			>
				<div class="flex min-h-0 flex-col overflow-hidden px-2 pb-2">
							...
							<div class="border-b border-gray-100 px-3 py-2.5 dark:border-gray-800">
								...
								{#if workspacePathTrail.length > 0}
									<div class="mt-2 overflow-x-auto">
										...
										>
											<div class="shrink-0 font-medium text-gray-500 dark:text-gray-400">
												{$i18n.t('Path')}
											</div>
											{#each workspacePathTrail as segment, index}
												<div class="shrink-0 text-gray-300 dark:text-gray-700">/</div>
												<button
													class="focus-ring touch-target-compact shrink-0 rounded-full px-2 py-0.5 transition {index ===
							...
							{#if generatedWorkspaceItems.length > 0}
								...
								>
									<svelte:fragment slot="actions">
										<button
											class="focus-ring touch-target-compact rounded-full bg-white px-2.5 py-1 text-[11px] font-medium text-gray-600 transition hover:bg-gray-100 dark:bg-gray-950 dark:text-gray-300 dark:hover:bg-gray-900"
											on:click={() =>
								...
								>
									<svelte:fragment slot="actions">
										<button
											class="focus-ring touch-target-compact rounded-full bg-white px-2.5 py-1 text-[11px] font-medium text-gray-600 transition hover:bg-gray-100 dark:bg-gray-950 dark:text-gray-300 dark:hover:bg-gray-900"
											on:click={() =>
												latestChatWorkspaceItem && selectItem(latestChatWorkspaceItem)}
				...
				>
					<div
						class="flex flex-wrap gap-1.5 border-b border-gray-100 px-3 py-2 dark:border-gray-800"
					>
						<button
							class="focus-ring touch-target-compact hidden rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-medium text-gray-600 transition hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 md:inline-flex"
							on:click={closePreview}
						>
							{$i18n.t('Close preview')}
						</button>
						<button
							class="focus-ring touch-target-compact rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-medium text-gray-600 transition hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
							on:click={openSelectedItem}
